package br.com.colbert.consolidador.infraestrutura.mail;
...
import br.com.colbert.consolidador.infraestrutura.info.*;
...
public class MailErroReporter implements ErroReporter, Serializable {
    ...
    public void reportar(Erro erro) {
        if (!Desktop.isDesktopSupported() || !Desktop.getDesktop().isSupported(Desktop.Action.MAIL) || GraphicsEnvironment.isHeadless()) {
        ...
        try {
            String mensagem = new MensagemEmailBuilder().para(emailsDesenvolvedores.get(0)).assunto(ASSUNTO_PADRAO)
                    .conteudo(erro.getMensagem() + StringUtils.repeat(StringUtils.LF, 2) + erro.getDetalhes()).anexo(new File(diretorioApp, "mychart.log").getAbsolutePath())
                    .build();
