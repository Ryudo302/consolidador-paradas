package br.com.colbert.consolidador.infraestrutura.swing;
...
import br.com.colbert.base.ui.ViewException;
import br.com.colbert.consolidador.infraestrutura.swing.model.ObjectTableModel;
...
public final class SwingUtils {
     ...
     *             caso a tabela permita a seleção de múltiplos elementos ou caso seu modelo não permita o acesso a um elemento a partir de seu índice
     */
    ...
    public static <E> Optional<E> getSelectedElement(JTable table) {
        ...
        if (selectedRow >= 0) {
            ...
            if (model instanceof ObjectTableModel) {
                return modelIndex != -1 ? (Optional<E>) Optional.of(((ObjectTableModel<?>) model).getElement(modelIndex)) : Optional.empty();
            } else {
                throw new IllegalArgumentException("Não é possível obter um elemento do modelo da tabela a partir de seu índice: " + modelIndex);
            }
        } else {
     ...
     *             caso ocorra algum erro durante a execução da tarefa ou ocorra alguma interrupção enquanto aguardava a execução da tarefa
