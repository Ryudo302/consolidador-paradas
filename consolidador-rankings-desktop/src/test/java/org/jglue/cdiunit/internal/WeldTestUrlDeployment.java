...
import org.jboss.weld.metadata.*;
...
public class WeldTestUrlDeployment implements Deployment {
    ...
    public WeldTestUrlDeployment(ResourceLoader resourceLoader, Bootstrap bootstrap, Class<?> testClass, Method testMethod) throws IOException {
        ...
        BeansXml beansXml;
        try {
            beansXml = new BeansXmlImpl(new ArrayList<Metadata<String>>(), new ArrayList<Metadata<String>>(), new ArrayList<Metadata<String>>(),
                    new ArrayList<Metadata<String>>(), Scanning.EMPTY_SCANNING, new URL("file:cdi-unit"), BeanDiscoveryMode.ANNOTATED, "cdi-unit", false);
        } catch (NoClassDefFoundError e) {
            try {
                beansXml = (BeansXml) BeansXmlImpl.class.getConstructors()[0].newInstance(new ArrayList<Metadata<String>>(), new ArrayList<Metadata<String>>(),
            ...
            if ((isCdiClass(c) || Extension.class.isAssignableFrom(c)) && !classesProcessed.contains(c) && !c.isPrimitive() && !classesToIgnore.contains(c)) {
                ...
                if (additionalClasspaths != null) {
                    for (Class<?> additionalClasspath : additionalClasspaths.value()) {
                        ...
                        Reflections reflections = new Reflections(new ConfigurationBuilder().setScanners(new TypesScanner())
                                .setUrls(new File(additionalClasspath.getProtectionDomain().getCodeSource().getLocation().getPath()).toURI().toURL()));

                        classesToProcess.addAll(ReflectionUtils.forNames(reflections.getStore().get(TypesScanner.class.getSimpleName()).keySet(),
                ...
                if (additionalPackages != null) {
                    for (Class<?> additionalPackage : additionalPackages.value()) {
                        ...
                        Reflections reflections = new Reflections(new ConfigurationBuilder().setScanners(new TypesScanner())
                                .setUrls(new File(additionalPackage.getProtectionDomain().getCodeSource().getLocation().getPath()).toURI().toURL())
